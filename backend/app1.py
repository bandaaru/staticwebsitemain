...
# ================= CONFIGURATION =================
MY_EMAIL = "agrifabrix@gmail.com"
MY_APP_PASSWORD = "qlrp xwhd elde ufxs"  # ⚠️ Move to env var in production
...
@static.route("/Contact", methods=["POST"])
def contact():
    db = get_mongo_db()
...
@static.route("/Subscribe", methods=["POST"])
def subscribe():
    db = get_mongo_db()
...
@static.route("/Onboarding", methods=["POST"])
def onboarding():
    db = get_mongo_db()   # get DB connection here
    ...
    try:
        ...
        send_simple_email(
            ...
            MY_EMAIL,
            get_html_template("New Partnership Request", f"<p>New onboarding request from {data['contactName']} <strong>{data['phone']}</strong> <strong>{data['email']}</strong> <strong>{data['partnershipReason']}</strong> <strong>{data['orgType']}</strong> </p>"),
             cc_emails=["info@agrifabrix.com"]
        )
...
@static.route("/Franchise", methods=["POST"])
def franchise():
    db = get_mongo_db()
...
@static.route("/Career/SendOTP", methods=["POST"])
def send_career_otp():
    db = get_mongo_db()
...
@static.route("/Career/VerifyOTP", methods=["POST"])
...
@static.route("/Career/Apply", methods=["POST"])
def career_apply():
    db = get_mongo_db()
